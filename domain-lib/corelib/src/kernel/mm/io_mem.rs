...
impl<const SIZE: usize> IoMem<SIZE> {
    ...
    pub unsafe fn try_new(res: Resource) -> Result<Self> {
        ...
        if res.offset % 8 != 0 {
            log::error!("Physical address is not 64-bit aligned: {:x}", res.offset);
            return Err(EDOM);
        }

        // Try to map the resource.
        // SAFETY: Just mapping the memory range.
        let addr = crate::sys_ioremap(res.offset, res.size as _);
        if addr.is_null() {
            Err(ENOMEM)
        } else {
...
impl<const SIZE: usize> Drop for IoMem<SIZE> {
    fn drop(&mut self) {
        ...
        // call to `ioremap`.
        crate::sys_iounmap(self.ptr as _);
    }
}
