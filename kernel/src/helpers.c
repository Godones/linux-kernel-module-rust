...
// folio
void rust_helper_folio_get(struct folio *folio) { folio_get(folio); }
void rust_helper_folio_put(struct folio *folio) { folio_put(folio); }
struct folio *rust_helper_folio_alloc(gfp_t gfp, unsigned int order){ return folio_alloc(gfp, order); }
struct page *rust_helper_folio_page(struct folio *folio, size_t n) { return folio_page(folio, n); }
loff_t rust_helper_folio_pos(struct folio *folio){ return folio_pos(folio); }

size_t rust_helper_folio_size(struct folio *folio) { return folio_size(folio); }
void rust_helper_folio_lock(struct folio *folio) { folio_lock(folio);}


bool rust_helper_folio_test_uptodate(struct folio *folio){ return folio_test_uptodate(folio); }
void rust_helper_folio_mark_uptodate(struct folio *folio){ folio_mark_uptodate(folio); }
bool rust_helper_folio_test_highmem(struct folio *folio) { return folio_test_highmem(folio); }
void rust_helper_flush_dcache_folio(struct folio *folio) { flush_dcache_folio(folio); }

void *rust_helper_kmap_local_folio(struct folio *folio, size_t offset) {
    return kmap_local_folio(folio, offset);
}
void rust_helper_kunmap_local(const void *vaddr) { kunmap_local(vaddr); }
...
// fs
sector_t rust_helper_bdev_nr_sectors(struct block_device *bdev) { return bdev_nr_sectors(bdev); }
struct dentry *rust_helper_dget(struct dentry *dentry) { return dget(dentry); }
loff_t rust_helper_i_size_read(const struct inode *inode) { return i_size_read(inode); }
void *rust_helper_alloc_inode_sb(struct super_block *sb,
                                 struct kmem_cache *cache, gfp_t gfp) { return alloc_inode_sb(sb, cache, gfp); }
void rust_helper_inode_lock_shared(struct inode *inode) { inode_lock_shared(inode); }
void rust_helper_inode_unlock_shared(struct inode *inode) { inode_unlock_shared(inode); }
void rust_helper_mapping_set_large_folios(struct address_space *mapping) { mapping_set_large_folios(mapping);}

unsigned int rust_helper_MKDEV(unsigned int major, unsigned int minor) { return MKDEV(major, minor); }

void rust_helper_i_uid_write(struct inode *inode, uid_t uid) { i_uid_write(inode, uid); }
void rust_helper_i_gid_write(struct inode *inode, gid_t gid) { i_gid_write(inode, gid); }
void rust_helper_set_delayed_call(struct delayed_call *call,
                                  void (*fn)(void *), void *arg) { set_delayed_call(call, fn, arg); }
struct file *rust_helper_get_file(struct file *f) { return get_file(f); }
unsigned int rust_helper_memalloc_nofs_save(void) { return memalloc_nofs_save(); }
void rust_helper_memalloc_nofs_restore(unsigned int flags) { memalloc_nofs_restore(flags); }
...
// readb/w
/* io.h */
u8 rust_helper_readb(const volatile void __iomem *addr) { return readb(addr); }
u16 rust_helper_readw(const volatile void __iomem *addr) { return readw(addr); }
u32 rust_helper_readl(const volatile void __iomem *addr){ return readl(addr); }
u64 rust_helper_readq(const volatile void __iomem *addr){ return readq(addr); }

void rust_helper_writeb(u8 value, volatile void __iomem *addr) { writeb(value, addr); }
void rust_helper_writew(u16 value, volatile void __iomem *addr) {writew(value, addr); }
void rust_helper_writel(u32 value, volatile void __iomem *addr) { writel(value, addr); }
void rust_helper_writeq(u64 value, volatile void __iomem *addr){ writeq(value, addr); }

u8 rust_helper_readb_relaxed(const volatile void __iomem *addr) { return readb_relaxed(addr); }
u16 rust_helper_readw_relaxed(const volatile void __iomem *addr) { return readw_relaxed(addr); }
u32 rust_helper_readl_relaxed(const volatile void __iomem *addr) { return readl_relaxed(addr); }
u64 rust_helper_readq_relaxed(const volatile void __iomem *addr) { return readq_relaxed(addr); }

void rust_helper_writeb_relaxed(u8 value, volatile void __iomem *addr) { writeb_relaxed(value, addr); }
void rust_helper_writew_relaxed(u16 value, volatile void __iomem *addr) { writew_relaxed(value, addr); }
void rust_helper_writel_relaxed(u32 value, volatile void __iomem *addr) { writel_relaxed(value, addr); }
void rust_helper_writeq_relaxed(u64 value, volatile void __iomem *addr) { writeq_relaxed(value, addr); }



// blk
unsigned int rust_helper_blk_rq_payload_bytes(struct request *rq) { return blk_rq_payload_bytes(rq); }
unsigned short rust_helper_blk_rq_nr_phys_segments(struct request *rq) { return blk_rq_nr_phys_segments(rq); }
bool rust_helper_req_ref_inc_not_zero(struct request *req) { return atomic_inc_not_zero(&req->ref); }
bool rust_helper_req_ref_put_and_test(struct request *req) { return atomic_dec_and_test(&req->ref); }
...
// device
const char *rust_helper_dev_name(const struct device *dev) { return dev_name(dev); }
