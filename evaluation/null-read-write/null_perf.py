...
# rel-LKM-c
# qd1-bs4k-jb1
# qd{QD}-bs{Block Size}-jb{Threads}
...
def get_null_test_data()->pd.DataFrame:
    # read data from pickle
    with open("nullrandread_iops.pkl", "rb") as f:
        ty_with_read_iops: dict = pickle.load(f)
    with open("nullrandwrite_iops.pkl", "rb") as f:
    ...
    for type_key in ty_with_read_iops.keys():
        for config_key in ty_with_read_iops[type_key].keys():
            if "rel-DM-c" in type_key or "rel-LKM-c" in  type_key:
                iops[type_key][config_key] = (ty_with_read_iops[type_key][config_key][0], ty_with_write_iops[type_key][config_key][1], ty_with_read_iops[type_key][config_key][2], ty_with_write_iops[type_key][config_key][3])
    ...
    for key in iops.keys():
        ...
        for config in value_dict.keys():
            ...
            final_data["Type"].extend(["randread", "randwrite"])
...
def draw(data:pd.DataFrame):
    ...
    color_mapping = {
        ...
        "unmodified Rust-6-thread": custom_colors[2],
        "domain-adapted-6-thread": custom_colors[3]
    }
    ...
    grid.map(
        ...
        dodge=True, 
        edgecolor="black",
        hue_order=["unmodified Rust-1-thread", "domain-adapted-1-thread", "unmodified Rust-6-thread", "domain-adapted-6-thread"],
    ...
    for (i,ty)in enumerate(["randread", "randwrite"]):
        for (j,qd) in enumerate(["1", "8", "32", "128"]):
            ...
            new_err_values = []
            # get Rust-1-thread, Domain-1-thread, Rust-6-thread, Domain-6-thread
            for blk_size in ["4k", "32k", "256k", "1m"]:
                for hue in ["unmodified Rust-1-thread", "domain-adapted-1-thread", "unmodified Rust-6-thread", "domain-adapted-6-thread"]:
            ...
            if j == 0 and i == 1:
                ax[i][j].set_ylabel("randwrite")
                # ax[i][j].set_ylim(-0.2, 0.2)
    
    
    # set title
    # grid.fig.suptitle("Null Blk Throughput, $(R - C) / C$ (Bare Metal)")
    ...
    grid.set_xlabels("")
    grid.set_xticklabels(labels=["4KiB","32KiB","256KiB","1MiB"], rotation=45)
    # show legend
    # two clumn and tow row
    # Rust-1-thread, Domain-1-thread
    # Rust-6-thread, Domain-6-thread
    grid.add_legend(ncol=2, frameon=True, bbox_to_anchor=(0.03, 0.98), loc="outside upper left")


    # 调整布局
    plt.tight_layout(rect=[0.03, 0.04, 1, 0.87])  # 为图例和标题留出空间

    plt.savefig("performance_comparison_null.svg", format="svg")
    plt.show()


if __name__ == "__main__":
    null_df = get_null_test_data()
    draw(null_df)