...
use crate::{bindings, error::*, types::Opaque, CStr};
...
/// use kbind::driver::net::phy::{self, DeviceId};
/// use kbind::{CStr,cstr};
/// use kmacro::vtable;
///
/// kbind::module_phy_driver! {
///     drivers: [PhySample],
///     device_table: [
...
///     const NAME: CStr<'static> = cstr!("PhySample");
...
macro_rules! module_phy_driver {
    ...
    (drivers: [$($driver:ident),+ $(,)?], device_table: [$($dev:expr),+ $(,)?], $($f:tt)*) => {
        struct Module {
            _reg: $crate::driver::net::phy::Registration,
        }
        ...
        const _: () = {
            ...
            impl $crate::KernelModule for Module {
                fn init() -> Result<Self> {
                    // SAFETY: The anonymous constant guarantees that nobody else can access
                    // the `DRIVERS` static. The array is used only in the C side.
                    let drivers = unsafe { &mut DRIVERS };
                    let mut reg = $crate::driver::net::phy::Registration::register(
                        module,
                        ::core::pin::Pin::static_mut(drivers),
                    )?;
                    Ok(Module { _reg: reg })
