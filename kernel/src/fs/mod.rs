...
use crate::{
    bindings,
    error::{from_result, linux_err::*, to_result, Error, KernelResult as Result},
    init::PinInit,
    mm::mem_cache::MemCache,
...
impl<T: FileSystem + ?Sized> Tables<T> {
    ...
    unsafe extern "C" fn fill_super_callback(
        ...
        from_result(|| {
            ...
            sb.s_xattr = &mut Tables::<T>::XATTR_HANDLERS[0];
...
#[pin_data]
pub struct FsModule<T: FileSystem + ?Sized> {
    #[pin]
    fs_reg: Registration,
    _p: PhantomData<T>,
}

impl<T: FileSystem + ?Sized + Sync + Send> InPlaceModule for FsModule<T> {
    fn init(module: &'static ThisModule) -> impl PinInit<Self, Error> {
        pr_info!("Registering file system: {}\n", T::NAME);
        try_pin_init!(Self {
...
///     fn fill_super(_: &mut SuperBlock<Self, sb::New>, _: Option<Mapper>) -> KernelResult {
///         todo!()
///     }
///     fn init_root(_sb: &SuperBlock<Self>) -> KernelResult<dentry::Root<Self>> {
///         todo!()
///     }
/// }
/// # }
...
macro_rules! module_fs {
    (type: $type:ty, $($f:tt)*) => {
        type ModuleType = $crate::fs::FsModule<$type>;
        $crate::module! {
            type: ModuleType,
            $($f)*
        }
