...
use crate::{
    ...
    mem::{ptable::MappingFlags, PhysPage},
};
...
impl DomainLoader {
    ...
    fn load_program(&mut self, elf: &ElfFile) -> LinuxResult<()> {
        elf.program_iter()
            ...
            .for_each(|ph| {
                ...
                for _ in 0..len / FRAME_SIZE {
                    let frame = Box::new(crate::mem::alloc_frame_trackers(1));
                ...
                unimplemented!("map_area_to_kernel");
            });
        Ok(())
    }
    ...
    fn relocate_dyn(&self, elf: &ElfFile) -> LinuxResult<()> {
        if let Ok(res) = relocate_dyn(elf, self.phy_start) {
            ...
            res.into_iter().for_each(|kv| {
                ...
                let addr = crate::mem::query_kernel_space(kv.0).unwrap();
    ...
    pub fn load(&mut self) -> LinuxResult<()> {
        ...
        let region_start = crate::mem::alloc_free_region(end_paddr.as_usize()).unwrap();
        trace!(
            "region range:{:#x}-{:#x}",
            region_start,
...
impl Drop for DomainLoader {
    fn drop(&mut self) {
        ...
        for meta in &self.regions {
            ...
            crate::mem::unmap_region_from_kernel(vaddr).unwrap();
        }
        // flush tlb
        // sfence_vma_all();
    }
