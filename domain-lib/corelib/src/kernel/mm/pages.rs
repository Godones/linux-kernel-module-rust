...
use crate::{
    bindings,
    kernel::error::{linux_err::*, KernelResult as Result},
    PAGE_SIZE,
};
...
impl<const ORDER: u32> Pages<ORDER> {
    ...
    pub fn new() -> Result<Self> {
        let pages = crate::sys__alloc_pages(
...
impl Pages<0> {
    ...
    pub fn kmap(&self) -> PageMapping<'_, NormalMappingInfo> {
        let ptr = crate::sys_kmap(self.pages);
    ...
    pub fn kmap_atomic(&self) -> PageMapping<'_, AtomicMappingInfo> {
        let ptr = crate::sys_kmap_atomic(self.pages);
...
impl<const ORDER: u32> Drop for Pages<ORDER> {
    fn drop(&mut self) {
        ...
        crate::sys__free_pages(self.pages, ORDER);
    }
}
...
impl MappingActions<AtomicMappingInfo> for Pages<0> {
    ...
    unsafe fn unmap(mapping: &PageMapping<'_, AtomicMappingInfo>) {
        ...
        crate::sys_kunmap_atomic(mapping.ptr);
    }
}
