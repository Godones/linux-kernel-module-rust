...
impl<T> PinnedDrop for Timer<T> {
    fn drop(self: Pin<&mut Self>) {
        ...
        crate::sys_hrtimer_cancel(self.timer.get());
    }
}
...
macro_rules! impl_has_timer {
    ($(impl$(<$($implarg:ident),*>)?
       ...
       { self.$field:ident }
        ...
        unsafe impl$(<$($implarg),*>)? $crate::kernel::time::hrtimer::HasTimer<$timer_type> for $self $(<$($selfarg),*>)? {
            const OFFSET: usize = ::core::mem::offset_of!(Self, $field) as usize;

            #[inline]
            unsafe fn raw_get_timer(ptr: *mut Self) -> *mut $crate::kernel::time::hrtimer::Timer<$timer_type $(, $id)?> {
