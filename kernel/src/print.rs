...
use crate::{bindings, str::RawFormatter};
...
macro_rules! println_color {
    ($color:expr, $fmt:expr) => {
        match $color {
            31 => $crate::pr_err!("[LKM] {}\n",$fmt),
            32 => $crate::pr_info!("[LKM] {}\n",$fmt),
            33 => $crate::pr_warn!("[LKM] {}\n", $fmt),
            34 => $crate::pr_debug!("[LKM] {}\n",$fmt),
            _ => $crate::pr_info!("[LKM] {}\n",$fmt),
        }
    };
    ($color:expr, $fmt:expr, $($arg:tt)*) => {
        match $color {
            31 => $crate::pr_err!(concat!("[LKM] ", $fmt, "\n"), $($arg)*),
            32 => $crate::pr_info!(concat!("[LKM] ", $fmt, "\n"), $($arg)*),
            33 => $crate::pr_warn!(concat!("[LKM] ", $fmt, "\n"), $($arg)*),
            34 => $crate::pr_debug!(concat!("[LKM] ", $fmt, "\n"), $($arg)*),
            _ => $crate::pr_info!(concat!("[LKM] ", $fmt, "\n"), $($arg)*),
        }
    };
}
