...
run_fio(){
  ...
  thread=$2
  bs=$3
  echo Run command: sudo fio --name=mytest --ioengine=psync --rw=${type} --bs=${bs} --numjobs=${thread} --time_based --runtime=${RUN_TIME} -filename=/dev/${CLASS} --size=4G -group_reporting --direct=1 --iodepth=128
  count=1
  while(( count<=3 ))
  do
    echo ----------TEST Impl:${CLASS} TYPE:$type THREAD:$thread BS:$bs $count start----------
    sudo fio --name=mytest --ioengine=psync --rw=${type} --bs=${bs} --numjobs=${thread} --time_based --runtime=${RUN_TIME} -filename=/dev/${CLASS} --size=4G -group_reporting --direct=1 --iodepth=128
    sync
