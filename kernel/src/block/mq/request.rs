...
impl<T: Operations> Request<T> {
    ...
    pub fn map_sg(&self, sglist: &mut [bindings::scatterlist]) -> Result<u32> {
        ...
        if sglist.len() < self.nr_phys_segments().into() {
            return Err(crate::error::linux_err::EINVAL);
        }
        ...
        if count < 0 {
            Err(crate::error::linux_err::ENOMEM)
        } else {
            Ok(count as _)
        }
    }
...
impl<T> hrtimer::RawTimerCallback for ARef<Request<T>>
where
    T: Operations,
    T::RequestData: HasTimer<T::RequestData>,
    ...
    T::RequestData: Sync,
{
    unsafe extern "C" fn run(ptr: *mut bindings::hrtimer) -> bindings::hrtimer_restart {
        // `Timer` is `repr(transparent)`
        let timer_ptr = ptr.cast::<hrtimer::Timer<T::RequestData>>();
...
unsafe impl<T: Operations> AlwaysRefCounted for Request<T> {
    fn inc_ref(&self) {
        ...
        let _updated = atomic_relaxed_op_unless(refcount, |x| x + 1, 0);
    ...
    unsafe fn dec_ref(obj: core::ptr::NonNull<Self>) {
        ...
        // #[cfg_attr(not(CONFIG_DEBUG_MISC), allow(unused_variables))]
        let _new_refcount = atomic_relaxed_op_return(refcount, |x| x - 1);
