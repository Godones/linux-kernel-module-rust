...
impl Allocator for CoherentAllocator {
    ...
    fn free(cpu_addr: *mut (), dma_handle: u64, size: usize, dev: &mut Device) {
        crate::sys_dma_free_attrs(dev.ptr, size, cpu_addr as _, dma_handle, 0);
    }
...
pub fn try_alloc_coherent<T>(
    ...
    let ret = crate::sys_dma_alloc_attrs(
        dev.raw_device(),
        size,
        &mut dma_handle,
        if atomic {
...
impl<T> Pool<T> {
    ...
    pub fn try_new(
        ...
        let size = count.checked_mul(t_size).ok_or(ENOMEM)?;
        let ptr =
            crate::sys_dma_pool_create(name.as_char_ptr(), dev.raw_device(), size, align, boundary);
        if ptr.is_null() {
            Err(ENOMEM)
        } else {
            Arc::try_new(Self {
    ...
    pub fn try_alloc(&self, atomic: bool) -> Result<CoherentAllocation<T, Self>> {
        ...
        let mut dma_handle = 0;
        let ptr = crate::sys_dma_pool_alloc(self.ptr, flags, &mut dma_handle);
        if ptr.is_null() {
            Err(ENOMEM)
        } else {
            Ok(CoherentAllocation::new(
...
impl<T> Allocator for Pool<T> {
    ...
    fn free(cpu_addr: *mut (), dma_handle: u64, _size: usize, pool: &mut *mut bindings::dma_pool) {
        crate::sys_dma_pool_free(*pool, cpu_addr as _, dma_handle);
    }
...
impl<T> Drop for Pool<T> {
    fn drop(&mut self) {
        ...
        crate::sys_dma_pool_destroy(self.ptr);
    }
}
