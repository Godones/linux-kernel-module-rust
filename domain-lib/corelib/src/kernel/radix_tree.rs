...
impl<V: ForeignOwnable> RadixTree<V> {
    ...
    pub fn get(&self, key: Key) -> Option<V::Borrowed<'_>> {
        // SAFETY: `self.tree` points to a valid and initialized `struct radix_tree`
        let item = core::ptr::NonNull::new(crate::sys_radix_tree_lookup(self.tree.get(), key))?;
    ...
    pub fn remove(&mut self, key: Key) -> Option<V> {
        // SAFETY: `self.tree` points to a valid and initialized `struct radix_tree`
        let item = core::ptr::NonNull::new(crate::sys_radix_tree_delete(self.tree.get(), key))?;
...
impl<V: ForeignOwnable> Drop for RadixTree<V> {
    fn drop(&mut self) {
        ...
        let mut slot = crate::sys_radix_tree_iter_init(&mut iter, 0);
        loop {
            if slot.is_null() {
                // SAFETY: Both `self.tree` and `iter` are valid
                slot = crate::sys_radix_tree_next_chunk(self.tree.get(), &mut iter, 0);
            }
            ...
            let item = crate::sys_radix_tree_delete(self.tree.get(), iter.index);
            assert!(!item.is_null());
            ...
            // not null.
            slot = crate::sys_radix_tree_next_slot(slot, &mut iter, 0);
        }
    }
}
